      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-3 py-2">
        {navItems.map((item) => (
          <Link
            key={item.to}
            to={item.to}
            className={`relative flex min-w-[60px] flex-col items-center gap-1 rounded-lg px-2 py-2 text-xs font-semibold transition-all duration-200 ${
              activePath(item.to)
        ...
        {!isAuthenticated && (
          <Link
            to="/login"
            className="flex min-w-[60px] flex-col items-center gap-1 rounded-lg px-2 py-2 text-xs font-semibold text-slate-600 hover:text-teal-700 hover:bg-slate-100 transition-all duration-200"
          >

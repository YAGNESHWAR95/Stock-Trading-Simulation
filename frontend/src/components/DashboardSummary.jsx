...
export default function DashboardSummary() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <p className="text-slate-400 font-semibold text-sm">Compiling platform telemetry and cash limits...</p>
      </div>
    );
  }
  ...
  if (!summary) {
    return (
      <div className="glass-panel p-8 rounded-2xl text-center border-rose-500/20 text-rose-400 py-12">
        ...
        <p className="text-sm text-slate-500 mt-1">Please ensure your backend service connection is active.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="glass-panel p-6 rounded-2xl flex items-center justify-between shadow-lg">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Available Cash</span>
            <span className="text-2xl font-display font-extrabold text-white font-mono block">
        ...
        <div className="glass-panel p-6 rounded-2xl flex items-center justify-between shadow-lg">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Invested Capital</span>
            <span className="text-2xl font-display font-extrabold text-white font-mono block">
        ...
        <div className="glass-panel p-6 rounded-2xl flex items-center justify-between shadow-lg">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Positions Valuation</span>
            <span className="text-2xl font-display font-extrabold text-white font-mono block">
              ${summary.totalCurrentValue?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
          </div>
          <div className="p-3 bg-blue-500/10 border border-blue-500/15 rounded-xl text-blue-400">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="glass-panel p-6 rounded-2xl flex items-center justify-between shadow-lg">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Net Return (P&L)</span>
            <span className={`text-2xl font-display font-extrabold font-mono block ${isNetReturnPositive ? "text-emerald-400 neon-text-green" : "text-rose-400 neon-text-red"}`}>
          ...
          <div className={`p-3 rounded-xl border ${isNetReturnPositive ? "bg-emerald-500/10 border-emerald-500/15 text-emerald-400" : "bg-rose-500/10 border-rose-500/15 text-rose-400"}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              {isNetReturnPositive ? (
      ...
      <div className="glass-panel rounded-2xl shadow-xl overflow-hidden">
        ...
        {/* Table Title Panel */}
        <div className="px-6 py-5 border-b border-white/5 bg-slate-900/20 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <svg className="w-5 h-5 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 10h18M3 14h18m-9-4v8m-7 0h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
            </svg>
            <h3 className="font-display font-extrabold text-white text-base">Open Equity Positions</h3>
          </div>
          <span className="bg-slate-900 border border-white/5 text-[10px] text-indigo-300 font-bold px-3 py-1 rounded-full uppercase tracking-wider">
        ...
        <div className="overflow-x-auto">
          {(!summary.portfolioBreakdown || summary.portfolioBreakdown.length === 0) ? (
            <div className="text-center py-16 space-y-3">
              ...
              <p className="text-slate-400 font-semibold">Your portfolio is currently empty.</p>
              <p className="text-xs text-slate-500">Visit the live market feed to initiate your first simulated asset transaction.</p>
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-900/50 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-white/5">
                    ...
                    <tr key={item.assetId} className="hover:bg-white/[0.02] transition duration-150">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-display font-bold text-white text-sm">{item.symbol}</span>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="inline-flex flex-col items-end">
                          <span className={`font-mono font-bold text-sm ${isPosPositive ? "text-emerald-400" : "text-rose-400"}`}>

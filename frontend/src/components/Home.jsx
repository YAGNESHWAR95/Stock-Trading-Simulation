    ...
    <div className="relative min-h-[80vh] flex flex-col items-center justify-center text-center px-4 py-12">
      ...
      <div className="max-w-4xl mx-auto space-y-8 relative z-10">
        ...
        <div className="space-y-4">
          <span className="inline-flex items-center gap-1.5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 font-bold text-xs uppercase tracking-widest px-4 py-1.5 rounded-full">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full max-w-md mx-auto pt-4">
          {isAuthenticated ? (
            ...
            <>
              ...
              <Link 
                to="/login" 
                className="w-full sm:w-auto bg-slate-900/50 hover:bg-slate-900 border border-white/10 text-slate-200 hover:text-white px-8 py-3.5 rounded-xl font-bold text-base transition duration-200 text-center cursor-pointer"
              >
                Log In
              </Link>
            </>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 pt-12 max-w-3xl mx-auto">
          ...
          <div className="glass-panel p-5 rounded-2xl border border-white/5 text-center space-y-2">
            <div className="w-10 h-10 bg-indigo-500/10 border border-indigo-500/15 rounded-xl flex items-center justify-center text-indigo-400 mx-auto">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <h3 className="font-display font-extrabold text-white text-sm">Demo Wallet</h3>
            <p className="text-xs text-slate-500 leading-normal">Practice paper trading with a default $100,000 risk-free account balance.</p>
          </div>

          <div className="glass-panel p-5 rounded-2xl border border-white/5 text-center space-y-2">
            <div className="w-10 h-10 bg-emerald-500/10 border border-emerald-500/15 rounded-xl flex items-center justify-center text-emerald-400 mx-auto">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
            <h3 className="font-display font-extrabold text-white text-sm">Real-Time Feeds</h3>
            <p className="text-xs text-slate-500 leading-normal">Track price updates streaming continuously via persistent WebSockets.</p>
          </div>
          ...
          <div className="glass-panel p-5 rounded-2xl border border-white/5 text-center space-y-2">
            ...
            <h3 className="font-display font-extrabold text-white text-sm">Groq AI Doubt Clearer</h3>
            <p className="text-xs text-slate-500 leading-normal">Consult a virtual assistant using Llama-3.3 on demand for strategy queries.</p>
          </div>
